...
public class LoginActivity extends MainFrameActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		initView("用户登录", "", true);
		et_username = (EditText) lock.findViewById(R.id.et_username);
		et_password = (EditText) lock.findViewById(R.id.et_password);
		et_password.setText(TicketApplication.password);
		et_username.setText(TicketApplication.username);
	}
	...
	public boolean validate() {
		...
		if (username.equals("")) {
			CreateAlertDialog("用户名不能为空", LoginActivity.this);
			return false;
		} else if (password.equals("")) {
			CreateAlertDialog("密码不能为空", LoginActivity.this);
			return false;
		}
		return true;
	}
	...
	public void run() {
		...
		if (NetworkCheck.IsHaveInternet(this)) {
			...
			TicketApplication.username = et_username.getText().toString();
			TicketApplication.password = et_password.getText().toString();
