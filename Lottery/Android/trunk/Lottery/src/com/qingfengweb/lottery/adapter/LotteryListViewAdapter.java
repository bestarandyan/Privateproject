...
//			holder.xingtaiTv = (TextView) convertView.findViewById(R.id.xingTaiTv);
			...
			holder.dateTv = (TextView) convertView.findViewById(R.id.dateTextView);
		...
		String currentDateStr = currentQi.substring(0, currentQi.length()-3);
		if(position>0){
			String preDateStr = list.get(position-1).get("ticket_no").toString().substring(0, currentQi.length()-3);
			if(currentDateStr.equals(preDateStr)){
				holder.dateTv.setVisibility(View.GONE);
			}else{
				holder.dateTv.setVisibility(View.VISIBLE);
				holder.dateTv.setText(currentDateStr.substring(0, 4)+"-"+currentDateStr.substring(4, 6)+"-"+currentDateStr.substring(6, 8));
			}
		}else{
			holder.dateTv.setVisibility(View.VISIBLE);
			holder.dateTv.setText(currentDateStr.substring(0, 4)+"-"+currentDateStr.substring(4, 6)+"-"+currentDateStr.substring(6, 8));
