...
public class DBHelper {
	 ...
	 * @param tv
	 */
	public void setBalance(TextView tv){
		if(tv == null){
			return;
		}
		String moneySql = "select available_balance from "+UserBalanceBean.tabName+" where username="+MyApplication.getInstance().getCurrentUserName();
		List<Map<String,Object>> moneyList = selectRow(moneySql, null);
		if(moneyList!=null && moneyList.size()>0){
			String balance = moneyList.get(0).get("available_balance").toString();
			if(balance!=null && balance.length()>0 && !balance.equals("0")){
				tv.setText("￥"+balance);
			}else{
				tv.setText("￥0.00");
			}
