/**
 * 
 */
package com.qingfengweb.weddingideas.share;

public class ConstantsValuesShare {

	// 新浪微博appkey
	public static final String SINA_CONSUMER_KEY = "3763226624";
	public static final String SINA_REDIRECT_URL = "http://www.weddingideas.cn/pages/WedArticle/aboutUsPage.do";
	public static final int SINA_TYPE = 1;

	// 腾讯微博appkey
	public static final String TENCENT_CONSUMER_KEY = "100589119";
	public static final String TENCENT_CONSUMER_SECRET = "6148334cd7e95896b3f96de80e9ed58c";
	public static final String TENCENT_REDIRECT_URL = "http://www.qingfengweb.com";
	public static final int TENCENTWEIBO_TYPE = 2;
	
	// 微信appkey
	public static String WECHAT_CONSUMER_ID = "wxe89d2dffc6cb6669";
	public static final int WECHAT_TYPE = 3;//微信客户端内容
	public static final int WECHAT_INFO_TYPE = 1;//微信
	public static final int WECHAT_FRIEND_TYPE = 2;//朋友网
	
	
	// qq开放平台appid
	public static final String TENCENT_ID = "100589119";
	public static final String TENCENT_KEY = "6148334cd7e95896b3f96de80e9ed58c";// qq开放平台appkey
	public static final int TENCENT_TYPE = 4;
	
	
	//分享到短信
	public static final int SMS_TYPE = 5;
	
	//分享到邮件
	public static final int EMAIL_TYPE = 6;
}
