...
public class VncCanvasActivity extends Activity {
	...
	class ZoomInputHandler extends AbstractGestureInputHandler {
		 ...
		 * 
		 * @see
		 * android.androidVNC.AbstactGestureInputHandler#onTouchEvent(android
		 * .view.MotionEvent)
		 */
...
//		
			if(e.getAction() == MotionEvent.ACTION_MOVE&&mIsTouch){
				return vncCanvas.pan(0, 0);	
			}else{
				return super.onTouchEvent(e);
			}
		}
	...
	public void onCreate(Bundle icicle) {
		...
		ExitApplication.getInstance().context = VncCanvasActivity.this;
		ExitApplication.getInstance().addActivity(VncCanvasActivity.this);
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		if (true) {
			getMenuInflater().inflate(R.menu.menu_vnc, menu);
		} else {
    			...
    			MyApplication.getInstance().getSocket().sendUrgentData(0xFF);
    			 MyApplication.getInstance().getSendToService().sDraw(data);
    				...
    				try {
    			        ...
    			        sendService.setDrawStyle(2,Color.BLACK);
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		case R.id.huaban:
			/*	Intent intent1=new Intent(this,MainActivity.class);
				...
				finish();*/
				vncCanvas.closeConnection();// liuxingxing
				Intent i = this.getIntent();
				int flag = i.getIntExtra("activity_flag", 0);
				Thread thread = new Thread(runnable);
			    thread.start();
				if(flag == 1){
					Intent intent2=new Intent();
		            VncCanvasActivity.this.setResult(22, intent2);
		            finish();
				}else{
					Intent intent2=new Intent();
						...
						.setPositiveButton("确定",
								new DialogInterface.OnClickListener() {
									public void onClick(DialogInterface dialog,
											int which) {
										vncCanvas.closeConnection();// liuxingxing
										Intent intent=new Intent(VncCanvasActivity.this,AndroidVNCEntry.class);
		...
		case R.id.itemFitToScreen:       
			if(!isFlag){
				...
				isFlag=false;
				AbstractScaling.getById(R.id.itemZoomable).setScaleTypeForActivity(
						this);
				item.setChecked(true);
				showPanningState();
			}
//            	 Constant.isFlag=true;
//            	vncCanvas.scaling.zoomOut(this);
...
//			/*
//			 * AbstractScaling.getById(item.getItemId()).setScaleTypeForActivity(
//			 * this); item.setChecked(true); showPanningState();
//			 */
//			return true;
		case R.id.itemCenterMouse:
			
			  vncCanvas.warpMouse(vncCanvas.absoluteXPosition +
			  vncCanvas.getVisibleWidth() / 2, vncCanvas.absoluteYPosition +
			  vncCanvas.getVisibleHeight() / 2);
		...
		case R.id.itemFollowMouse:
			 ...
			 if (newFollow) { vncCanvas.panToMouse(); }
			 connection.save(database.getWritableDatabase());
		...
		default:
			/*
			 * AbstractInputHandler input =
			 * getInputHandlerById(item.getItemId()); if (input != null) {
			 * inputHandler = input; connection.setInputMode(input.getName());
			 * if (input.getName().equals(TOUCHPAD_MODE))
			 * connection.setFollowMouse(true); item.setChecked(true);
			 * showPanningState();
			 * connection.save(database.getWritableDatabase()); return true; }
			 */
		}
