...
public class VncCanvas extends ImageView {
	...
	void initializeVncCanvas(ConnectionBean bean, final Runnable setModes) {
		...
		final ProgressDialog pd = ProgressDialog.show(getContext(), "连接中...", "请等待...", true, true, new DialogInterface.OnCancelListener() {
			@Override
			public void onCancel(DialogInterface dialog) {
				closeConnection();
				handler.post(new Runnable() {
					public void run() {
						Utils.showErrorMessage(getContext(), "VNC 连接终止!");
					}
				});
			}
		});
				...
				try {
					...
					if (maintainConnection) {
						...
						if (e instanceof OutOfMemoryError) {
							...
							final String error_ = error + "<br>" + "\t请检查：1、是否开启服务器；"+"\t"+"2、服务器设置是否正确；"+"\t"+"3、无线设备是否开启。";
							handler.post(new Runnable() {
								public void run() {
	...
	{
		float scale = getScale();
		float miage=(float)getWidth();
		float miage1=(float)getHeight();
		float iamgeWidth=getImageWidth();
		float iamgeHeight=getImageHeight();
		int x=(int)((absoluteXPosition + ((float)getWidth() - getImageWidth()) / 2 ) * scale);
		int y=(int)((absoluteYPosition + ((float)getHeight() - getImageHeight()) / 2 ) * scale);
	...
	boolean processPointerEvent(int x, int y, int action, int modifiers, boolean mouseIsDown, boolean useRightButton) {
		if (rfb != null && rfb.inNormalProtocol) {
		    if (action == MotionEvent.ACTION_DOWN || (mouseIsDown && action == MotionEvent.ACTION_MOVE)) {
		    	System.out.println(" 当前动作"+(action == MotionEvent.ACTION_MOVE?"move":"down"));
		      if (useRightButton) {
