...
import com.boluomi.children.adapter.TreasureChesViewAdapter;
...
import com.boluomi.children.util.DomAnalysisXml;
		...
		for(int i=0;i<list.size();i++){
			...
			list.get(i).put("imgDrawable", 
					res.getIdentifier(//这里是根据图片在drawable中的名字获取该图片的资源id
							listTreasure.get(0).get(map.get("id")).toString().replace(".png", "").trim(),//得到图片的名字，这里是不能直接写图片名称的，因为图片名称是后台给的，根据id去xml配置文件中获取
