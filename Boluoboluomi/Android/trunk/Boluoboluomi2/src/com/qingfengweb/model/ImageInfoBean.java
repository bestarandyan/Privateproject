package com.qingfengweb.model;

public class ImageInfoBean {
	//shouyaoName xinlangName xinniangName year  month ri week
	//longliyue longliri shijian dianming address phone
public String id;
public String shouyaox;
public String shouyaoy;
public String xinlangx;
public String xinlangy;
public String xinniangx;
public String xinniangy;
public String yearx;
public String yeary;
public String yuex;
public String yuey;
public String rix;
public String riy;
public String timex;
public String timey;
public String dianmingx;
public String dianmingy;
public String addressx;
public String addressy;
public String phonex;
public String phoney;
public String longliyuex;
public String longliyuey;
public String longlirix;
public String longliriy;
public String weekx;
public String weeky;
public String getYearx() {
	return yearx;
}
public void setYearx(String yearx) {
	this.yearx = yearx;
}
public String getYeary() {
	return yeary;
}
public void setYeary(String yeary) {
	this.yeary = yeary;
}
public String getYuex() {
	return yuex;
}
public void setYuex(String yuex) {
	this.yuex = yuex;
}
public String getYuey() {
	return yuey;
}
public void setYuey(String yuey) {
	this.yuey = yuey;
}
public String getRix() {
	return rix;
}
public void setRix(String rix) {
	this.rix = rix;
}
public String getRiy() {
	return riy;
}
public void setRiy(String riy) {
	this.riy = riy;
}
public String getLongliyuex() {
	return longliyuex;
}
public void setLongliyuex(String longliyuex) {
	this.longliyuex = longliyuex;
}
public String getLongliyuey() {
	return longliyuey;
}
public void setLongliyuey(String longliyuey) {
	this.longliyuey = longliyuey;
}
public String getLonglirix() {
	return longlirix;
}
public void setLonglirix(String longlirix) {
	this.longlirix = longlirix;
}
public String getLongliriy() {
	return longliriy;
}
public void setLongliriy(String longliriy) {
	this.longliriy = longliriy;
}
public String getWeekx() {
	return weekx;
}
public void setWeekx(String weekx) {
	this.weekx = weekx;
}
public String getWeeky() {
	return weeky;
}
public void setWeeky(String weeky) {
	this.weeky = weeky;
}
public String getId() {
	return id;
}
public void setId(String id) {
	this.id = id;
}
public String getShouyaox() {
	return shouyaox;
}
public void setShouyaox(String shouyaox) {
	this.shouyaox = shouyaox;
}
public String getShouyaoy() {
	return shouyaoy;
}
public void setShouyaoy(String shouyaoy) {
	this.shouyaoy = shouyaoy;
}
public String getXinlangx() {
	return xinlangx;
}
public void setXinlangx(String xinlangx) {
	this.xinlangx = xinlangx;
}
public String getXinlangy() {
	return xinlangy;
}
public void setXinlangy(String xinlangy) {
	this.xinlangy = xinlangy;
}
public String getXinniangx() {
	return xinniangx;
}
public void setXinniangx(String xinniangx) {
	this.xinniangx = xinniangx;
}
public String getXinniangy() {
	return xinniangy;
}
public void setXinniangy(String xinniangy) {
	this.xinniangy = xinniangy;
}
public String getTimex() {
	return timex;
}
public void setTimex(String timex) {
	this.timex = timex;
}
public String getTimey() {
	return timey;
}
public void setTimey(String timey) {
	this.timey = timey;
}
public String getDianmingx() {
	return dianmingx;
}
public void setDianmingx(String dianmingx) {
	this.dianmingx = dianmingx;
}
public String getDianmingy() {
	return dianmingy;
}
public void setDianmingy(String dianmingy) {
	this.dianmingy = dianmingy;
}
public String getAddressx() {
	return addressx;
}
public void setAddressx(String addressx) {
	this.addressx = addressx;
}
public String getAddressy() {
	return addressy;
}
public void setAddressy(String addressy) {
	this.addressy = addressy;
}
public String getPhonex() {
	return phonex;
}
public void setPhonex(String phonex) {
	this.phonex = phonex;
}
public String getPhoney() {
	return phoney;
}
public void setPhoney(String phoney) {
	this.phoney = phoney;
}




}
