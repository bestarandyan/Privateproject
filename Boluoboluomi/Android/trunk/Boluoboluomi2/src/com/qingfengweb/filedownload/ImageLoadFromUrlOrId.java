...
//				Drawable drawable = loaDrawable(imageUrl);
				Bitmap bitmap = loadImageFromUrl(imageUrl,toSD,context,defId);
				if(bitmap!=null){
...
//			System.out.println("这一次的网络下载文件名为-------------"+nameString);
	 ...
	 * @return
	 */
	private  Bitmap loadImageFromUrl(String imageUrl,String toSD,Context context,int defId){
		URL myFileUrl = null;
		Bitmap bitmap = null;
		String nameString = "";
		try {
...
//				System.out.println("图片存储========================"+(b?"成功":"失败"));
	...
	public Bitmap loadImageFromId(final Context context,final String interfaceStr,final List<NameValuePair> list,
		...
		new Thread(){
			public void run() {
				Bitmap bitmap = loadImageFromId(context,interfaceStr,list,toSD,defId,imgW,imgH);
				if(bitmap!=null){
