...
import com.chinaLife.claimAssistant.thread.sc_RequestServerFromHttp;
 ...
 * performance.
 */
public class sc_ImageDownloaderId {
	private static final String LOG = "ImageDownloader";
	int connection_count = 0;
	int connection_count_totle = 0;
	Context context;
	Bitmap bitmap = null;
	int resid = R.drawable.sc_icon_default_photo;

	public sc_ImageDownloaderId(Context context,int connection_count_totle) {
		this.context = context;
		this.connection_count_totle = connection_count_totle;
	}
	public sc_ImageDownloaderId(Context context,int resid,int connection_count_totle) {
		this.context = context;
		this.resid = resid;
		this.connection_count_totle = connection_count_totle;
	}
	 ...
	 */
	public void download(File file, String id, ImageView imageView) {
		...
		if (bitmap == null) {
			...
			forceDownload(file, id,imageView);
		} else {
...
//			imageView.setImageResource(resid);
	...
	public Bitmap downloadBitmap(File file, String id) {
		System.out.println("进入到方法中区1");
		Bitmap bitmap = null;
		if (file.exists() && file.isFile()) {
			try {
				bitmap = sc_CommonUtils.getDrawable(file.getAbsolutePath(), null);
			} catch (Exception e) {
			}
			if (bitmap != null) {
				return bitmap;
		...
		connection_count++;
		boolean b = sc_RequestServerFromHttp.downloadPhoto(file, id);
