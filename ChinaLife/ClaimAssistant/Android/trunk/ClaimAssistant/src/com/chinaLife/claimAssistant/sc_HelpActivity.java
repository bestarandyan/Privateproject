...
import com.chinaLife.adapter.sc_ArticleListAdapter;
...
public class sc_HelpActivity extends Activity implements OnItemClickListener,
		OnClickListener {
	...
	private void selectNotice() {
		if (sc_NetworkCheck.IsHaveInternet(this)
				&& sc_MyApplication.getInstance().isHelpupdate()) {
			...
			sc_UploadData uploaddata = new sc_UploadData(sc_MyApplication.URL
					+ "claim", params, 12);
			uploaddata.Post();
	...
	private void getListdata() {
		...
		for (int i = 0; i < selectresult.size(); i++) {
			...
			map.put("title", selectresult.get(i).get("title").toString());
	...
	public void parseJson() {
		...
		helpinfos = gson.fromJson(sc_MyApplication.getInstance()
		...
		for (Iterator<sc_HelpInfo> iterator = helpinfos.iterator(); iterator
			...
			if (selectresult.size() <= 0) {
				...
				sc_DBHelper.getInstance().update("helpinfo", values,
						"helpid = ?", new String[] { helpinfo.getHelpid() });
