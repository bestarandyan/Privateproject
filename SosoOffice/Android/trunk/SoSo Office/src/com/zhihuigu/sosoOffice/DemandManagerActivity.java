 ...
 *	需求管理类
 */
public class DemandManagerActivity extends BaseActivity implements Activity_interface,OnItemClickListener,IXListViewListener{
	private Button backBtn,addDemandBtn;//返回按钮   添加需求按钮
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.a_demandmanager);
		findView();//
		initView() ;
		initData();
	...
	public void onClick(View v) {
		if(v == backBtn){
			OnBack(MainActivity.class);
		}else if(v == addDemandBtn){
			MyApplication.getInstance().setDemandBack(true);
	...
	public void findView() {
		...
		addDemandBtn = (Button) findViewById(R.id.addDemandBtn);
	...
	public void initView() {
		...
		addDemandBtn.setOnClickListener(this);
	}
	@Override
	public void initData() {
		DBHelper.getInstance(this).execSql("update soso_countinfo set rcount=0 where userid = '"
	+MyApplication.getInstance().getSosouserinfo(this).getUserID()+"'");
	...
	public void getListData(){
		...
		String sql = "select * from soso_cusreleaseinfo where userid = '"
				+ MyApplication.getInstance(this).getSosouserinfo(this)
						.getUserID() + "' order by releaseid desc";
	...
	public Runnable runnable = new Runnable() {
		...
		public void run() {
			...
			if (NetworkCheck.IsHaveInternet(DemandManagerActivity.this)) {
				boolean b = false;
		...
		public void handleMessage(Message msg) {
			...
			case 3:// 登录失败给用户提示
				...
				if (StringUtils.getErrorState(reponse).equals(
					...
					errormsg = "您暂时还没有发布任何需求，根据您的需求我们将会反馈给业主，请点击右上角的加号，赶紧发布您的需求吧";
				} else {
					errormsg = getResources().getString(
...
//				Toast.makeText(DemandManagerActivity.this.getParent(), errormsg, Toast.LENGTH_SHORT).show();
				break;
			case 4:// 没有网络时给用户提示
				MessageBox.CreateAlertDialog(DemandManagerActivity.this
						.getParent());
				listView.stopRefresh();
				break;
			case 5:// 打开进度条
				progressdialog = new ProgressDialog(
						DemandManagerActivity.this.getParent());
	...
	public boolean getCurRelease() {
		...
		params.add(new BasicNameValuePair("whereClause", "UserID = "+MyApplication
...
//					"unit text"+//单位
					if (soSoCusReleaseInfo != null
						...
						if (DBHelper
								.getInstance(DemandManagerActivity.this)
								.selectRow(
										"select * from soso_cusreleaseinfo where releaseid = '"
												+ soSoCusReleaseInfo.getReleaseID()
												+ "'", null).size() <= 0) {
							DBHelper.getInstance(DemandManagerActivity.this)
